...
- A folder containing flow velocity Rasters (any Raster type is permitted) corresponding to multiple discharges ([read more on discharge definitions](#inputs)). A *Raster string* may be defined to select only Rasters that contain certain letters in their name from the defined folder (e.g., enter `u`).
- A folder containing flow depth Rasters (any Raster type is permitted) corresponding to multiple discharges ([read more on discharge definitions](#inputs)).  A *Raster string* may be defined to select only Rasters that contain certain letters in their name from the defined folder (e.g., enter `h`).
...
| Line 13 | Grain ssize| One raster name defining the raster containing mean grain diameters (typically `dmean`; pay attention to raster units: use feet for U.S. customary and m for S.I.)|
